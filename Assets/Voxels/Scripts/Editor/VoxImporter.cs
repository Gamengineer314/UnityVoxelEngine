...
namespace Voxels.Editor {
    ...
    /// <summary>
    /// .vox importer for models exported from Magica Voxel with the "vox" option
    /// Format : https://github.com/ephtracy/voxel-model/blob/master/MagicaVoxel-file-format-vox.txt
    /// </summary>
    [ScriptedImporter(1, "vox")]
    public class VoxImporter : VoxelImporter {
        protected override Native3DArray<Color32> ReadVoxels(string path) {
            ...
            if (Encoding.ASCII.GetString(reader.ReadBytes(4)) != "VOX ")
                throw new InvalidDataException("Missing 'VOX' header");
            reader.ReadInt32();

            // MAIN chunk
            if (Encoding.ASCII.GetString(reader.ReadBytes(4)) != "MAIN")
            ...
            if (id == "PACK") {
                ...
                if (reader.ReadInt32() != 1)
                    throw new InvalidDataException($"Files containing multiple models aren't supported");
            ...
            if (Encoding.ASCII.GetString(reader.ReadBytes(4)) != "XYZI")
                throw new InvalidDataException("Missing 'XYZI' chunk");
            reader.ReadInt32();
            reader.ReadInt32();
            ...
            // Optional RGBA chunk
            Color32[] palette = BuildDefaultPalette();
            if (reader.BaseStream.Position < reader.BaseStream.Length) {
                if (Encoding.ASCII.GetString(reader.ReadBytes(4)) == "RGBA") {
                    reader.ReadInt32();
                    reader.ReadInt32();
                    for (int i = 0; i < 255; i++) {
        ...
        private Color32[] BuildDefaultPalette() {
            uint[] raw = {
