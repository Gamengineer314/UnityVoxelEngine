...
namespace Voxels.Rendering {
    ...
    /// </summary>
    internal class MeshGenerator : ParallelGenerator<GenerationCommand, MeshGeneratorJob> {
        ...
        protected override IEnumerable<MeshGeneratorJob> CreateJobs(GenerationCommand command, int jobHorizontalSize) {
            ...
            if (buffers.ContainsCommand(command)) return null;

            List<MeshGeneratorJob> jobs = new();
            ...
            for (int jobZ = 0, i = 0; jobZ < nJobsZ; jobZ++) {
                for (int jobX = 0; jobX < nJobsX; jobX++, i++) {
                    ...
                    jobs.Add(new MeshGeneratorJob(
                        command.voxels,
                        jobStartX, jobStartZ, jobSizeX, jobSizeZ,
                        command.chunkSize, command.mergeNormalsThreshold, command.seenFromAbove, command.textured
                    ));
                }
            }
            return jobs;
        ...
        protected override void ProcessResult(GenerationCommand command, MeshGeneratorJob job) {
            buffers.AddData(command, job.chunks, job.faces, job.colors);
        }
    }
    ...
    [BurstCompile]
    internal struct MeshGeneratorJob : IJob, IDisposable {
        ...
        private NativeHashMap<int, int> colorIndices;
        ...
        private UnsafeArray<int2> chunkRanges;
            ...
            for (int chunkZ = 0; chunkZ < nChunksZ; chunkZ++) {
                ...
                for (int chunkX = 0; chunkX < nChunksX; chunkX++) {
                        ...
                        for (int x = currentChunkStart.x; x < chunkEndX; x++) {
                            ...
                            foreach (Voxel voxel in voxels.GetColumn(x, z)) {
                                if (!textured && !Voxel.Color32Equals(voxel.color, Voxel.ghost)) {
                                    ...
                                    if (!colorIndices.ContainsKey(id)) {
                                        colorIndices[id] = nIDs++;
                    ...
                    for (int chunkY = 0; chunkY < nChunksY; chunkY++) {
                        ...
                        // Generate all chunks
                        chunkRanges = new UnsafeArray<int2>(nSubChunksX * nSubChunksY * nSubChunksZ * 6, Allocator.Temp);
                        chunkIndex = 0;
            ...
            for (int y = 0; y < currentSubChunkSize[VoxelNormals.HeightAxis(axis)]; y++) {
                ...
                for (int x = 0; x < currentSubChunkSize[VoxelNormals.WidthAxis(axis)]; x++) {
                    ...
                    while (faceRow != 0) {
                        ...
                        int index = textured ? 0 : colorIndices[Voxel.Color32HashCode(color)];
        ...
        private void GenerateNormalOptimizedChunk(VoxelNormal normal) {
            ...
            chunkRanges[chunkIndex + (int)normal] = new int2(startFace, currentFaces.Length);
        }
        ...
        private void GenerateChunk() {
            ...
            while (normalIndex < 6) {
                ...
                while (faceCount < VoxelRenderer.maxFaceCount) {
                    if (faceIndex == chunkRanges[chunkIndex].y) { // Next chunk
                        chunkIndex += 6;
                        if (chunkIndex >= chunkRanges.length) { // Next normal
                            normalIndex++;
                            ...
                            if (faceCount > 0 && currentFaces.Length > mergeNormalsThreshold) { // Generate chunk for this normal
                                faceIndex = chunkRanges[chunkIndex].x;
                                break;
                            }
                        }
                        faceIndex = chunkRanges[chunkIndex].x;
                    ...
                    if (textured) {
                        if (colors.Length - startColor + width * height - 1 > VoxelFace.maxColor) break;
                        faces.Add(new VoxelFace(pos, width, height, normal, colors.Length - startColor));
                        for (int y = 0; y < height; y++) {
                            for (int x = 0; x < width; x++) {
                                ...
                                colors.Add(voxels.GetVoxel(texturePos));
                            }
                        }
                    }
                    else faces.Add(face);
